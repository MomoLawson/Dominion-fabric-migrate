...
/**
 * Limitation configuration - defines per-group limits for dominion creation.
 * Ported from Bukkit to Fabric/SnakeYAML.
 */
@Headers({
        "This file controls how player can use the dominion feature.",
...
public class Limitation extends ConfigurationFile {
    ...
    public static class LimitationText extends ConfigurationPart {
        ...
        public String loadWorldSettingFailed = "Failed to load world-settings of {0} reason: {1}";
    ...
    @Comments({
            ...
            "You need to install vault and an economy plugin to use this feature."
    })
    ...
    public static class WorldLimitationSetting extends ConfigurationPart {
        ...
        @Comments("Whether to include all vertical blocks when calculating the size.")
    ...
    public void checkWorldLimitationSettings() {
        ...
        if (!contains(worldLimitationsKey)) {
            createSection(worldLimitationsKey);
        }
    }
    ...
    public void loadWorldLimitationSettings() {
        ...
        Map<String, Object> section = getConfigurationSection(worldLimitationsKey);
        if (section == null) {
            return;
        }
        for (String key : section.keySet()) {
            ...
            try {
                ...
                ConfigurationManager.readConfigurationPart(getYamlData(), settings, worldLimitationsKey + "." + key);
    ...
    public void saveWorldLimitationSettings() {
        for (Map.Entry<String, WorldLimitationSetting> entry : worldLimitations.entrySet()) {
            try {
                ...
                ConfigurationManager.writeConfigurationPart(getYamlData(), entry.getValue(), prefix, !Objects.equals(entry.getKey(), "default"));
            } catch (Exception e) {
